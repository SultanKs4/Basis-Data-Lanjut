...
-- Soal 13
SELECT e.empid, e.lastname, e.firstname, e.title, e.mgrid,
	mgr.lastname AS mgrlastname, mgr.firstname AS mgrfirstname
FROM HR.Employees AS e, HR.Employees AS mgr
WHERE e.mgrid = mgr.empid;
GO
...
-- Soal 20
SELECT emp.empid, emp.firstname, emp.lastname, cal.calendardate
FROM HR.Employees AS emp
	CROSS JOIN HR.Calendar AS cal;
GO
...
-- Soal 30
SELECT cust.custid AS ID, cust.companyname AS [Company Name]
...
WHERE o.orderdate > '2008-04-01' OR o.orderdate = '2008-04-01'
...
FROM Sales.Orders
ORDER BY orderdate DESC 
OFFSET 0 ROW FETCH FIRST 20 ROWS ONLY;
GO

-- Soal 40
SELECT prod.productname AS [Product Name], prod.unitprice AS [Unit Price]
...
SELECT TOP(10) PERCENT
	prod.productname AS [Product Name], prod.unitprice AS [Unit Price]
FROM Production.Products AS prod
ORDER BY [Unit Price] DESC;
GO

-- Soal 43
SELECT ord.custid, ord.orderid, ord.orderdate
